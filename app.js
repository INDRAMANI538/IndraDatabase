...
// CONSTANTS & GLOBALS
// ============================================================
const ADMIN_REGISTRATION_CODE = 'ADMIN@SMS2024';
const PAGE_SIZE = 10;
...
let studentsCache      = [];      // local cache of fetched students
...
// ============================================================
// AUTH — FIREBASE AUTH STATE OBSERVER
// ============================================================
...
// ============================================================
// SIDEBAR — build nav items based on role
// ============================================================
...
function navigateTo(page) {
  ...
  document.getElementById('page-title').textContent =
    page === 'dashboard' ? 'Dashboard' : 'All Students';
...
async function renderDashboard() {
  try {
    ...
    studentsCache  = students;                // populate cache for modal clicks
    ...
    document.getElementById('page-content').innerHTML = `
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          <div class="stat-icon cyan">
            <svg viewBox="0 0 24 24"><path d="M12 3L1 9l4 2.18V15c0 2.43 3.61 5 7 5s7-2.57 7-5v-3.82L22 9 12 3zm6 8.99L12 15 6 12V9l6-3.27L18 9v2.99z"/></svg>
          </div>
          <div class="stat-info">
        ...
        <div class="stat-card">
          ...
          <div class="stat-info">
            <div class="stat-value">${alumni}</div>
      ...
      <div class="section-header">
        ...
        <div style="display:flex;gap:8px">
          ${addBtn}
          <button class="btn-small" onclick="navigateTo('students')">View All →</button>
        </div>
      </div>
                  ...
                  <td>
                    <div class="student-cell">
                      ...
                      <div>
                        ...
                        <div class="student-id">${escHtml(s.studentId || '')}</div>
                      </div>
                    </div>
                  </td>
    ...
    document.getElementById('page-content').innerHTML =
      `<div class="empty-state"><p style="color:var(--error)">Error loading dashboard. Check your Firebase config.</p></div>`;
  }
}
  ...
  document.getElementById('page-content').innerHTML = `
    ...
    <div class="search-bar">
      <div class="search-input-wrapper">
        ...
        <input type="text" id="search-input" placeholder="Search by name, ID, email, course…" oninput="handleSearch(this.value)" value="${escHtml(searchQuery)}" />
      </div>
...
function renderStudentsList() {
  ...
  if (currentView === 'table') {
    const adminCols = currentUserRole === 'admin'
      ...
      const adminCells = currentUserRole === 'admin' ? `
        ...
        <td>
          <div class="table-actions">
            <button class="btn-icon edit"   onclick="event.stopPropagation(); openEditModal('${s.id}')"   title="Edit">
              <svg viewBox="0 0 24 24"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>
            </button>
            <button class="btn-icon delete" onclick="event.stopPropagation(); openDeleteModal('${s.id}', '${escAttr(s.fullName)}')" title="Delete">
        ...
        <div class="student-card" onclick="openViewModal('${s.id}')">
          <div class="student-card-header">
            ...
            <div class="student-card-info">
              <h4>${escHtml(s.fullName)}</h4>
              <p>${escHtml(s.studentId || '')}</p>
              <p>${escHtml(s.email || '')}</p>
            </div>
          </div>
...
async function handleStudentSubmit(e) {
  ...
  try {
    if (editingStudentId) {
      ...
      const studentId = generateStudentId(all);
...
async function confirmDelete() {
  ...
  try {
    await db.collection('students').doc(deletingStudentId).delete();
    ...
    btn.disabled    = false;
    btn.textContent = '🗑️ Delete Student';
  }
}
...
function openEditModal(studentId) {
  ...
  if (!s) { showToast('Student data not found. Please refresh.', 'error'); return; }
...
function openViewModal(studentId) {
  ...
  if (s) {
    ...
    // Fetch directly if not in cache (e.g. from dashboard)
  ...
  document.getElementById('view-modal-body').innerHTML = `
    ...
    <div class="profile-details-grid">
      <div class="profile-detail-item">
        <label>Phone</label><span>${escHtml(s.phone || 'Not provided')}</span>
      </div>
      <div class="profile-detail-item">
        <label>Date of Birth</label><span>${s.dob ? formatDate(s.dob) : 'Not provided'}</span>
      </div>
      <div class="profile-detail-item">
        <label>Gender</label><span>${escHtml(s.gender || 'Not provided')}</span>
      </div>
      <div class="profile-detail-item">
        <label>Course / Department</label><span>${escHtml(s.course || 'Not provided')}</span>
      </div>
      <div class="profile-detail-item">
        <label>Semester</label><span>${escHtml(s.semester || 'Not provided')}</span>
      </div>
      <div class="profile-detail-item">
        <label>GPA</label><span>${s.gpa != null ? s.gpa + ' / 10' : 'Not provided'}</span>
      </div>
      <div class="profile-detail-item">
        <label>Enrollment Date</label><span>${s.enrollmentDate ? formatDate(s.enrollmentDate) : 'Not provided'}</span>
      </div>
      <div class="profile-detail-item">
        <label>Status</label><span>${escHtml(s.status || 'Active')}</span>
      </div>
      <div class="profile-detail-item full">
        <label>Address</label><span>${escHtml(s.address || 'Not provided')}</span>
      </div>
    </div>
    ${adminActions}
  `;
...
document.getElementById('student-modal-overlay')?.addEventListener('click', e => { if (e.target === e.currentTarget) closeStudentModal(); });
document.getElementById('view-modal-overlay')?.addEventListener('click',    e => { if (e.target === e.currentTarget) closeViewModal(); });
...
function statusOptions(selected) {
  return ['Active','Inactive','Alumni','Suspended'].map(s =>
